...
Sources
-------
- CubeSat Design Specification (CDS) Rev 14, Cal Poly, 2014
  https://www.cubesat.org/cubesatinfo
- PocketQube Standard Issue 1.7, 2018
  https://static1.squarespace.com/static/53d7dcdce4b07a1cdbbc08a4/
  t/5b34c395352f5303fcec6f45/1530184648111/PocketQube+Standard+iss1.7.pdf

Height values for CubeSat use the *maximum structural envelope* along the
deployment (Z) axis as defined in CDS Rev 14 Table 2-1, which includes the
...
#   Minimum natural frequency (all axes): 100 Hz (CDS Rev 14 §3.4.1)
# ---------------------------------------------------------------------------
...
CUBESAT_FORM_FACTORS: Dict[str, CubeSatFormFactor] = {
    "0.5U": CubeSatFormFactor(
        ...
        min_mass_kg=0.10, typical_mass_kg=0.30, max_mass_kg=0.67,
        min_freq_hz=100.0,
    ),
    "1U": CubeSatFormFactor(
        ...
        min_mass_kg=0.10, typical_mass_kg=1.00, max_mass_kg=1.33,
        min_freq_hz=100.0,
    ),
    "1.5U": CubeSatFormFactor(
        ...
        min_mass_kg=0.30, typical_mass_kg=1.50, max_mass_kg=2.00,
        min_freq_hz=100.0,
    ),
    "2U": CubeSatFormFactor(
        ...
        min_mass_kg=0.50, typical_mass_kg=2.00, max_mass_kg=2.66,
        min_freq_hz=100.0,
    ),
    "3U": CubeSatFormFactor(
        ...
        min_mass_kg=1.00, typical_mass_kg=3.50, max_mass_kg=4.00,
        min_freq_hz=100.0,
    ),
    "3U+": CubeSatFormFactor(
        name="3U+ CubeSat",
...
# ---------------------------------------------------------------------------
# PocketQube Form Factors  (PocketQube Standard Issue 1.7)
...
POCKETQUBE_FORM_FACTORS: Dict[str, CubeSatFormFactor] = {
    ...
    "2p": CubeSatFormFactor(
        name="2p PocketQube",
        units=2.0,
        width_mm=50.0, depth_mm=50.0, height_mm=100.0,
    ...
    "3p": CubeSatFormFactor(
        name="3p PocketQube",
        units=3.0,
        width_mm=50.0, depth_mm=50.0, height_mm=150.0,
